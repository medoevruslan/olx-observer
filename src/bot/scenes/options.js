...
const { createRegex } = require("../../handlers/regexHandler");
    ...
    [
      {
        ...
        callback_data: `GET ALL RESULTS:${queryId}`,
      },
    ],
  ]).resize();
};
...
optionsScene.on("text", async (ctx) => {
  try {
    ...
    if (!/^\d{3,5}$/.test(ctx.message.text))
      await ctx.reply("нужна адекватная цена");
    const toEdit = ctx.scene.state.data.edit;
    const newPrice = ctx.message.text;
    const newPriceText = toEdit.text.replace(
      /max price: \d{3,5}/,
      `*__max price: ${newPrice}__*`
    );
...
optionsScene.action(/^get all results:\d*$/i, async (ctx) => {
  try {
    ...
    cards.forEach(async (card) => await ctx.reply(card.link));
  } catch (err) {
    console.log(err);
  }
});
  ...
  try {
    ...
    if (queries.length > 0) {
      await queries.forEach(async (qry) => {
        ...
        const regex = createRegex(qry);
        await ctx.replyWithHTML(
          pretifyString(
            `Поиск по запросу: ${qry.searchQuery}
regex: (${regex.brand}) and (${regex.model})    *${
              qry.regexForModel ? "ON" : "OFF"
            }*
max price: ${qry.maxPrice}`
          ),
          queryKeyboard(qry.id, cards.length, qry.regexForModel)
        );
      });
    }
...
function editMessage(ctx, chatId, messageId, newText, btnId, switcher) {
  ctx.telegram.editMessageText(chatId, messageId, undefined, newText, {
    reply_markup: {
      inline_keyboard: [
        [
